  ...
  return [
    {
      ...
      async config(config2, env) {
        const { manifest: _manifest, offscreen } = await getOptions(config2);
          ...
          if (manifest2.content_scripts)
            for (const script of manifest2.content_scripts) {
              script.js = script.js?.map((id) => {
                return id.endsWith("?static") ? id.split("?")[0] : getFileName({ id, type: "loader" });
              });
            }
        } else {
          if (manifest2.background && "service_worker" in manifest2.background) {
...
Public dir: "${config.publicDir}"`
        ...
        if (config.command === "serve" && (files.html.length || offscreenHtml)) {
          const refId2 = this.emitFile({
            type: "asset",
          ...
          files.html.concat(offscreenHtml).forEach(
            (f) => f && this.emitFile({
              type: "asset",
              fileName: f,
